  ...
  <v-card elevation="10" class="withbg"
    ...
    <v-card-item class="pa-0">
      <div class="d-sm-flex align-center justify-space-between">
        ...
        <div class="ma-7">
          <v-btn prepend-icon="mdi-plus" variant="outlined" rounded="xl" style="color: rgb(var(--v-theme-primary));">Add
            new</v-btn>
        </div>
      </div>
      <v-container class="">
        <v-row justify="start">
          <v-col v-for="cat in cats" :key="cat.id" cols="3">
            <v-card elevation="10" @click.stop="editItem(cat)">
              <div class="py-5 px-4">
                <v-row justify="start">
                  <v-col cols="2" class="mx-2" style="background-color: aqua;border-radius: 50%; min-width: fit-content;">
                    <h4>{{ cat.icon }}</h4>
                  </v-col>
                  <v-col class="pl-2">
                    <h5>{{ cat.name }} </h5>
